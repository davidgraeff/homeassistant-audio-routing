<script lang="ts">
  ...
  // places from one source: as a side card next to the graph on wide screens, and
  // inside the graph's help dialog on narrow ones. It used to be two paragraphs
  // printed above and below the graph, which is where nobody reads them.
  interface Props {
...
<div class="help">
  ...
  {#if anyLatency || anyXruns}
    <h3>Badges</h3>
    {#if anyLatency}
      <p>
        The <strong>~ms</strong> figures estimate the buffering each node adds — the configured jitter/playout buffer,
        not a live measurement. A route's rough latency ≈ its <em>source</em> figure + its <em>speaker</em> figure, plus
        a little fixed graph overhead (sync anchor and one processing quantum, ~50–150&nbsp;ms) and the sender's own
        buffer (a phone's Bluetooth stack, say), which aren't shown. Tune the input jitter buffer and per-device
        alignment under <strong>Sources</strong>, the sendspin group lead under <strong>Settings</strong>.
      </p>
    {/if}
    {#if anyXruns}
      <p>
        A <strong class="xrun-inline">⚠ count</strong> is that node's dropped audio cycles (PipeWire xruns, like
        <code>pw-top</code>'s ERR). It turns red while climbing, marking where a dropout originates. Only collected
