...
It did **not** change the silence symptom (that was item 0, now fixed) — but it is
...
### 5. The device's API log subscription delivers nothing
...
### 6. The firmware pin is four months stale — but the ladder is the cost
...
### 7. §4.10's residual single-device gap
...
group-wide restart, much smaller window, one device. A rapid nudge burst in the Align tab
can extend it, which is what the tab's warning is for.

### 8. Frontend copy edits were never type-checked
...
### 9. `.claude/` is untracked and unignored
