...
pub(crate) fn handle_announce(
    ...
    // With a key present, fill key+iv for AES-CBC. With none present, leave both
    // all-zero: an unencrypted stream (aeskey stays zero => no decryption).
    if let Some(key_bytes) = key_bytes {
        if key_bytes.len() >= 16 {
            aeskey.copy_from_slice(&key_bytes[..16]);
        }
        if let Some(iv_str) = aesiv_str {
            if let Ok(iv_bytes) = conn.shared.rsakey.decode(iv_str) {
                if iv_bytes.len() >= 16 {
