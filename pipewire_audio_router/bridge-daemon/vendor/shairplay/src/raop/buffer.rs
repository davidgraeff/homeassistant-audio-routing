...
impl RaopBuffer {
    ...
    pub fn queue(&mut self, data: &[u8], use_seqnum: bool) -> i32 {
        ...
        // (RAOP et=0, or auth_setup which sends no key material); pass the
        // payload through undecrypted in that case.
        let unencrypted = self.aeskey == [0u8; RAOP_AESKEY_LEN];
        if encrypted_len > 0 && !unencrypted {
        ...
        let num_samples = match &mut self.codec {
            Codec::Alac { decoder, .. } => {
                ...
                let n = (output_size / 2).min(capacity);
            ...
            Codec::Pcm { .. } => {
                ...
                let n = (packet_buf.len() / 2).min(capacity);
                if n == 0 {
                    return 0;
                }
                let out = &mut self.entries[idx].audio_buffer;
                for i in 0..n {
                    let s = i16::from_be_bytes([packet_buf[i * 2], packet_buf[i * 2 + 1]]);
                    out[i] = s as f32 / 32768.0;
                }
