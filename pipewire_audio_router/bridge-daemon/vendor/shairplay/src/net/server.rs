    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    loop {
        ...
        while request.is_complete() {
            ...
            // Dump the raw serialized response (plaintext before encryption) so
            // the status line + reply headers (Public, Transport, ...) are visible.
            if !handler.is_encrypted() {
                tracing::debug!(
                    "RTSP <<< {method} {url} [{status}]\n{resp}",
                    resp = String::from_utf8_lossy(response.get_data()).trim_end()
                );
            }
