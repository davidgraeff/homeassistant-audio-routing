...
/// Try to set real-time priority for the async **producer** thread (Linux only).
///
/// `run_streamer` (the per-packet ALAC encode + encryption that feeds the
/// dedicated `rt-sender` thread) runs on its own OS thread rather than the
/// shared Tokio pool — see [`spawn_producer_thread`]. On a CPU-saturated host
/// the shared normal-priority pool starved it (observed `Decode took
/// 40-380ms (blocking send loop!)`), which in turn starved the FIFO-50 sender
/// (the producer couldn't keep the `bounded(8)` channel full). Priority 48 sits
/// just below the sender's 50 (so precise egress timing still wins) and above
/// the daemon's capture/relay feeders and any normal-priority noise (e.g. mDNS),
/// so the producer preempts that instead of being preempted by it. Best-effort:
...
fn set_producer_realtime_priority() {
    ...
    unsafe {
        ...
        if libc::sched_setscheduler(0, libc::SCHED_FIFO, &param as *const _) == 0 {
            tracing::info!("ap2-producer: real-time priority set (SCHED_FIFO, priority 48)");
        } else {
            tracing::warn!(
                "ap2-producer: could not set RT priority (need CAP_SYS_NICE): errno={}",
                *libc::__errno_location()
            );
        }
    }
...
pub struct AudioStreamer {
    ...
    /// Dedicated `SCHED_FIFO` OS thread running `run_streamer` (the encode/encrypt
    /// producer) on its own current-thread runtime — see [`spawn_producer_thread`].
    /// Replaces the former `tokio::spawn` on the shared pool so a saturated host
    /// can't starve the producer (and, through it, the sender).
    producer_thread: Option<std::thread::JoinHandle<()>>,
    /// Monotonic stop signal for the producer thread. Set once by [`stop`] and
    /// only ever *read* by `run_streamer` — unlike `state_cache`/`inner.state`
    /// (which run_streamer itself mutates during Buffering/underrun), so it can't
    /// be raced back to a non-stopped value, guaranteeing the thread joins.
    /// A fresh flag is installed on each `start`/`start_live` so a restart works.
...
impl Clone for AudioStreamer {
    fn clone(&self) -> Self {
        Self {
            ...
            producer_thread: None, // Can't clone JoinHandle; the clone doesn't own the thread
...
impl AudioStreamer {
    ...
    pub async fn start(&mut self, decoder: AudioDecoder) -> Result<()> {
        ...
        if self.producer_thread.is_none() {
            ...
            // Producer on a dedicated SCHED_FIFO thread (was tokio::spawn on the
            // shared pool) so a saturated host can't starve encode → sender. Install
            // a fresh stop flag so a restart after a prior stop() begins un-stopped.
            self.stop_flag = Arc::new(AtomicBool::new(false));
            self.producer_thread = Some(spawn_producer_thread(
                self.inner.clone(),
                self.stop_flag.clone(),
                self.state_cache.clone(),
                self.timestamp_cache.clone(),
    ...
    pub async fn start_live(&mut self, live_decoder: LiveAudioDecoder) -> Result<()> {
        ...
        // For live streaming, wait for a SMALL initial buffer before streaming.
        // CRITICAL: a large prefill sends NO audio and NO PT=87 anchors for its
        // whole duration after RECORD — real AP2 receivers (Yamaha/Pioneer) then
        // fail to lock their render clock and render SILENCE (proven: a 50%/~873ms
        // prefill = silent; the file path starts ~100ms after RECORD and plays).
        // The receiver's render buffer (render_delay, ~1.5s) is the real jitter
        // buffer — the sender only needs a couple of packets buffered to start.
        tracing::info!("Live streaming: waiting for initial buffer fill...");
        let buffer_start = std::time::Instant::now();
        let max_wait = std::time::Duration::from_secs(5);
        let target_fill_pct = 1.0; // ~20ms — start almost immediately after RECORD
        ...
        loop {
            ...
            // Small delay before retry (short: we want to start ~immediately)
    ...
    pub async fn stop(&mut self) -> Result<()> {
        // Signal the producer to break its loop. `stop_flag` is monotonic and
        // read-only for run_streamer, so (unlike state_cache/inner.state, which
        // run_streamer itself mutates) this can't be raced back to running. Set
        // state_cache too for any external observers.
        self.stop_flag.store(true, Ordering::Relaxed);
        self.state_cache.store(StreamerState::Stopped as u8, Ordering::Relaxed);

        // Best-effort Stop to the sender thread (may not fit a full channel;
        // it also exits on channel disconnect once the producer drops its tx).
        if let Some(ref tx) = self.sender_tx {
            let _ = tx.try_send(SenderMessage::Stop);
        }

        // Join the producer FIRST, while the sender thread is still draining the
        // bounded channel — otherwise a producer parked on a full `tx.send`
        // (backpressure) would never observe the stop. On exit the producer drops
        // its own `sender_tx` clone, so the channel can then fully disconnect.
        ...
        // Now drop our sender_tx: with the producer gone, this disconnects the
        // channel and guarantees the sender thread's rx.recv() returns Err and it
        // exits, even if the Stop message above couldn't be delivered.
        self.sender_tx = None;
...
/// Launch [`run_streamer`] on a dedicated `SCHED_FIFO` OS thread with its own
/// current-thread Tokio runtime, instead of `tokio::spawn` on the shared pool.
///
/// `run_streamer` is not truly cooperative: per 8 ms packet it does a synchronous
/// ALAC encode + encryption and blocks on `LiveAudioDecoder`'s `recv_timeout`
/// (≤2 ms/frame) and on the `bounded(8)` crossbeam send backpressure. On the
/// shared normal-priority pool a saturated host stalled it for tens to hundreds
/// of ms (`Decode took … (blocking send loop!)`), starving the FIFO-50 sender.
/// A dedicated RT thread fixes that; a current-thread runtime is used because
/// the body still `.await`s a cross-runtime `tokio::sync::Mutex`, `sleep`, and
/// `yield_now` (all runtime-agnostic — no work is spawned onto this runtime).
///
/// The thread exits on its own when `inner.state` becomes `Stopped`/`Error`
/// (set by [`AudioStreamer::stop`]); the returned handle is then joined there.
fn spawn_producer_thread(
    ...
    std::thread::Builder::new()
        .name("ap2-producer".into())
        .spawn(move || {
            set_producer_realtime_priority();
            let rt = match tokio::runtime::Builder::new_current_thread().enable_all().build() {
                Ok(rt) => rt,
                Err(e) => {
                    tracing::error!("ap2-producer: failed to build runtime: {e}");
                    state_cache.store(StreamerState::Error as u8, Ordering::Relaxed);
                    return;
                }
            };
            rt.block_on(async move {
                match run_streamer(inner.clone(), stop_flag, state_cache.clone(), timestamp_cache, packets_sent, underruns, sender_tx).await {
        ...
        .expect("Failed to spawn ap2-producer thread")
}
...
async fn run_streamer(
    ...
    loop {
        // Monotonic stop signal from `AudioStreamer::stop` — checked first and
        // independently of `inner.state` (which this loop itself mutates), so a
        // stop can never be raced back to a running state and the thread always
        // exits and joins.
...

        {
            ...
            if let Some(frame) = frame {
                ...
                // PT=87 anchors must be in the grandmaster's CLOCK_MONOTONIC
                // timeline (libairptp) — NOT the epoch wall clock used for NTP.
                // CLOCK_MONOTONIC in ns — the same timeline the daemon's libairptp
                // grandmaster advertises (PT=87 anchors must match it). Inlined
                // here after libairptp was removed from this vendored copy.
                let ptp_mono_ns = {
                    let mut ts = libc::timespec { tv_sec: 0, tv_nsec: 0 };
                    unsafe { libc::clock_gettime(libc::CLOCK_MONOTONIC, &mut ts) };
                    (ts.tv_sec as u64 * 1_000_000_000 + ts.tv_nsec as u64) + guard.render_delay_ns
                };
                    ...
                    if let Some(ref tx) = sender_tx {
                        ...
                        let sync_data = if need_sync {
                            if use_ptp_sync {
                                ...
                                guard.rtp_senders[0].prepare_ptp_sync(rtp_ts, ptp_mono_ns, next_rtp_ts, &ptp_clock_id)?
                            } else {
                        ...
                        // Direct crossbeam send — NOT spawn_blocking. Dispatching a
                        // blocking-pool task per 8 ms packet caused pool-contention
                        // latency spikes (observed 70–330 ms in run_streamer) that
                        // starved the RT sender thread → outbound audio gaps → live-path
                        // dropout. The bounded(8) still backpressures: this send blocks
                        // only briefly, when the sender thread is momentarily behind
                        // (≤ one burst); the yield_now below keeps the loop cooperative.
                        match tx.send(msg) {
                            Ok(()) => {}
                            Err(_) => {
                        ...
                        if need_sync {
                            if use_ptp_sync {
                                let next_rtp_ts = rtp_ts.wrapping_add(sample_rate / 44100 * 352);
                                guard.rtp_senders[0].send_ptp_sync(rtp_ts, ptp_mono_ns, next_rtp_ts, &ptp_clock_id)?;
                            } else {
        ...
        if has_sender_thread {
            ...
            // provides natural backpressure - when it's full, the direct crossbeam
            // send above throttles us to match the sender thread's consumption rate.
            // This keeps the channel maximally filled so the sender thread never
            // starves.
            //
            // Yield to let other Tokio tasks run (NTP, control, etc.) — this is the
            // loop's cooperative yield point (the send is now synchronous).
