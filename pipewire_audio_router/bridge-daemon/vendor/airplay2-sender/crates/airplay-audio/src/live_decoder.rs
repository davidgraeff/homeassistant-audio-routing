...
pub struct LiveFrameSender {
    ...
    /// Free-list of drained sample buffers returned by the decoder. A producer on a
    /// hot/RT path (e.g. the AP2 capture→feed relay) calls `take_buffer` to reuse a
    /// buffer's capacity instead of allocating a fresh `Vec<i16>` every chunk.
    recycle_rx: Receiver<Vec<i16>>,
}

impl LiveFrameSender {
    /// Take a recycled (emptied) sample buffer to fill and pass to `try_send`,
    /// or a fresh empty one if the free-list is empty. The decoder returns drained
    /// buffers to this list, so in steady state the producer does **no** allocation.
...
pub struct LiveAudioDecoder {
    ...
    /// Returns drained sample buffers to the sender's free-list (see
    /// `LiveFrameSender::take_buffer`). Bounded; on a full list the buffer is just
    /// freed. `None` disables recycling (e.g. `LiveAudioDecoder::new` without a pair).
    recycle_tx: Option<Sender<Vec<i16>>>,
    sample_rate: u32,
    channels: u8,
...
impl LiveAudioDecoder {
    ...
    pub fn create_pair(sample_rate: u32, channels: u8, capacity: usize) -> (LiveFrameSender, Self) {
        ...
        // Buffer recycle free-list: bounded so a stalled decoder can't grow it without
        // bound (a full list just frees the returned buffer). Sized > channel depth so
        // every in-flight buffer has a slot to come back to.
        let (recycle_tx, recycle_rx) = bounded::<Vec<i16>>(capacity * 2 + 8);
        let sender = LiveFrameSender { tx, recycle_rx };
        let mut decoder = Self::new(rx, sample_rate, channels);
        decoder.recycle_tx = Some(recycle_tx);
        (sender, decoder)
    }
            ...
            match self.decode_frame()? {
                Some(frame) => {
                    if source_rate == target_rate {
                        // Copy (not move) so the producer's buffer survives to be
                        // recycled below.
                        collected_samples.extend_from_slice(&frame.samples);
                    } else {
                    ...
                    // Return the producer's sample buffer to the sender's free-list so
                    // a hot/RT producer (the AP2 relay) reuses it instead of allocating
                    // a fresh Vec per chunk. Bounded try_send: if full, just drop/free.
                    if let Some(ref tx) = self.recycle_tx {
