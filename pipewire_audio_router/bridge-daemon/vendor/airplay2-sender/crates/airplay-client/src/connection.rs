...
pub struct Connection {
    ...
    /// Receiver→sender ("read") key, kept so the reverse event channel — over
    /// which the receiver reports its own volume changes — can be decrypted
    /// (same HomeKit-framed ChaCha20-Poly1305 as RTSP, a fresh nonce stream).
    events_read_key: Option<[u8; 32]>,
    /// Background task reading the event channel and forwarding device-reported
    /// volume (0.0–1.0) to whoever called [`Connection::volume_events`].
    events_task: Option<JoinHandle<()>>,
    /// Grandmaster PTP clock identity. In the daemon this is injected via
    /// set_ptp_clock_id() from the host-global libairptp; it is embedded in every
    /// PT=87 audio-sync packet so the receiver correlates the shared timeline.
...
impl Connection {
    ...
    pub async fn connect_with_pin(device: Device, config: StreamConfig, pin: &str) -> Result<Self> {
        ...
        // Keep the read key for the reverse event channel (volume feedback).
        let events_read_key = Some(*session_keys.read_key.as_bytes());
    ...
    pub async fn setup(&mut self) -> Result<()> {
        ...
        // Bounded connect (3s), mirroring OwnTone and the RECORD timeout below. An
        // unreachable events port (e.g. Pioneer silently dropping the SYN) must NOT
        // stall SETUP on the kernel's ~127s SYN-retry timeout — that would invalidate
        // the whole session and leave the receiver silent. Non-fatal either way.
        // event_port == 0 (not advertised in the SETUP reply) → skip the connect.
        if event_port == 0 {
            warn!("Events port not advertised (0); skipping events connection");
        } else {
            match tokio::time::timeout(
                ...
                Err(_) => {
                    warn!("Events port {} connect timed out after 3s (proceeding anyway)", events_addr);
                }
            }
        }
        ...
        // RECORD must be acknowledged (200) for the receiver to actually enter the
        // playing state. A cold or stale-session receiver (its previous session not
        // yet released) often times out / errors here and then renders NOTHING —
        // previously we "continued anyway" and streamed to a dead session, so the
        // receiver stayed silent until a manual reconnect. Instead FAIL: the caller
        // (ap2_server::connect_one) retries a FRESH connect, which typically succeeds
        // once the receiver is warm / the old session has dropped. 5 s window (was 2)
        // gives a cold receiver time to answer on the first try.
        match tokio::time::timeout(
            ...
            Ok(Ok(resp)) => {
                if resp.status_code == 200 {
                    ...
                    warn!("RECORD returned status {} — failing setup to force a fresh retry", resp.status_code);
                    return Err(RtspError::UnexpectedStatus(resp.status_code).into());
                }
            }
            Ok(Err(e)) => {
                warn!("RECORD error — failing setup to force a fresh retry: {}", e);
                return Err(e);
            }
            Err(_) => {
                warn!("RECORD timed out — failing setup to force a fresh retry");
            ...
            TimingProtocol::Ptp => {
                match self.stream_config.ptp_mode {
                    airplay_core::PtpMode::Master => {
                        // Daemon-integrated mode: the daemon owns the single host-global
                        // libairptp gPTP grandmaster (binds 319/320 once, drives every
                        // receiver as a PTP peer) and injects its clock id via
                        // set_ptp_clock_id() BEFORE setup(). This Connection must NOT start
                        // its own PTP or bind 319/320 — it only streams RTP whose PT=87
                        // anchors carry the injected grandmaster clock id on the shared
                        // CLOCK_MONOTONIC timeline. (The original standalone-spike paths that
                        // ran libairptp / a BMCA yield here were removed for the daemon copy.)
                        let (offset_tx, _offset_rx) = watch::channel(ClockOffset::default());
                        self.timing_tx = Some(offset_tx);
                        self.timing_offset = Some(ClockOffset::default());
                        if self.ptp_master_clock_id.is_none() {
                            tracing::warn!(
                                "PTP Master: no external clock id set — call set_ptp_clock_id() before setup(); PT=87 would carry a zero clock id"
                            );
                        }
                    }
    ...
    pub async fn disconnect(&mut self) -> Result<()> {
        ...
        // Stop the event-channel reader (volume feedback)
    ...
    /// Subscribe to **device-reported** volume changes (0.0–1.0), parsed from the
    /// receiver's reverse event channel — e.g. a user turning the AVR's own knob.
    ///
    /// Spawns a background reader on the first call (after [`Self::setup`], which
    /// establishes the event channel). Returns a receiver that yields whenever the
    /// receiver sends a `SET_PARAMETER volume: <dB>` over that channel. If the
    /// channel or its key is unavailable the receiver simply never yields (the
    /// sender is dropped immediately). The reader is best-effort and fail-safe: any
    /// read/decrypt/parse error just ends it and never affects streaming. It is
    /// aborted by [`Self::disconnect`].
    ///
    /// NOTE: The receiver's exact event-channel framing/encryption is
    /// hardware-specific and could not be validated here without the physical
    ...
    pub fn volume_events(&mut self) -> mpsc::UnboundedReceiver<f32> {
        ...
        match (self.events_stream.take(), self.events_read_key) {
            ...
            _ => {
                tracing::debug!("AP2 volume-event feedback unavailable (no event channel or key)");
                // tx dropped here → rx yields None immediately.
            }
        }
        rx
    }
    ...
    pub async fn start_streaming_live(&mut self, live_decoder: LiveAudioDecoder) -> Result<()> {
        ...
        // Enable PT=87 PTP sync on the LIVE path too (start_streaming does this;
        // the live path originally omitted it). Uses the injected grandmaster id.
        if self.stream_config.timing_protocol == TimingProtocol::Ptp {
            if let Some(clock_id) = self.ptp_master_clock_id {
                streamer.set_ptp_sync_mode(clock_id).await;
            }
    ...
    /// Retune the render delay on the LIVE stream (and store it for any re-start).
    /// Unlike [`Self::set_render_delay_ms`], this propagates to the running streamer
    /// so a UI change takes effect mid-stream — no reconnect. The new offset applies
    /// to the next PT=87 anchor the streamer emits.
    pub async fn set_render_delay_live(&mut self, delay_ms: u32) {
        self.render_delay_ms = delay_ms;
        if let Some(ref mut s) = self.streamer {
            s.set_render_delay_ms(delay_ms).await;
        }
    }
    ...
    /// Read the receiver's CURRENT volume as 0.0–1.0, or `None` if it doesn't
    /// answer `GET_PARAMETER volume` (support is receiver-specific). The receiver's
    /// own volume is authoritative — the caller uses this to show the real level
    /// instead of imposing one. AirPlay volume is dB (`20·log10(v)`, `-144`=mute);
    /// we invert to linear. Time-bounded + fail-safe: any error/timeout ⇒ `None`.
    pub async fn get_volume(&mut self) -> Option<f32> {
        let req = RtspRequest::get_parameter_text(self.session.request_uri(), b"volume\r\n".to_vec());
        let resp = tokio::time::timeout(std::time::Duration::from_secs(3), self.rtsp.send(req))
            .await
            .ok()?
            .ok()?;
        if !resp.is_success() {
        ...
        let text = std::str::from_utf8(resp.body.as_deref()?).ok()?;
        // Body is "volume: <dB>\r\n".
        let db: f32 = text
            .lines()
            .find_map(|l| l.trim().strip_prefix("volume:").map(str::trim))
            .and_then(|v| v.parse().ok())?;
        // dB → linear amplitude; clamp (-144 mute ⇒ ~0, 0 dB ⇒ 1.0).
        let lin = 10f32.powf(db / 20.0);
        Some(lin.clamp(0.0, 1.0))
    }
    ...
    pub async fn setup_as_ptp_master(&mut self, all_peer_ips: &[IpAddr]) -> Result<()> {
        ...
        let events_addr = SocketAddr::new(addr, event_port);
        tracing::info!("Establishing events connection to {}", events_addr);
        // Bounded connect (3s) — see the note in `setup()`: an unreachable events port
        // must not stall SETUP on the kernel SYN-retry timeout. Non-fatal.
        if event_port == 0 {
            warn!("Events port not advertised (0); skipping events connection");
        } else {
            match tokio::time::timeout(
                std::time::Duration::from_secs(3),
                tokio::net::TcpStream::connect(events_addr),
            ).await {
                Ok(Ok(stream)) => {
                    tracing::info!("Events connection established");
                    self.events_stream = Some(stream);
    ...
    pub async fn setup_for_group(
        ...
        // must not stall SETUP on the kernel SYN-retry timeout. Non-fatal.
        if event_port == 0 {
            warn!("Events port not advertised (0); skipping events connection (group member)");
        } else {
            match tokio::time::timeout(
                std::time::Duration::from_secs(3),
                TcpStream::connect(events_addr),
    ...
    /// Inject the host-global grandmaster clock id (the daemon's libairptp).
    /// Call BEFORE `setup()` so the PTP-master path and every PT=87 audio-sync
    /// packet carry it. The daemon owns 319/320; this Connection never binds them.
    pub fn set_ptp_clock_id(&mut self, clock_id: u64) {
        self.ptp_master_clock_id = Some(clock_id.to_be_bytes());
    }

    /// Get the PTP master clock ID (if set).
...
/// Convert an AirPlay volume in dB back to the 0.0–1.0 linear scale — the
/// inverse of the send path's `20*log10(volume)` (see airplay-rtsp
/// `build_set_volume`). `0 dB` = full scale; the AirPlay "muted" sentinel
/// (−144 dB) and anything at/below the −30 dB floor map to `0.0`.
...
/// Find the most recent `volume: <dB>` line in decrypted event-channel text and
/// return it as a 0.0–1.0 level. `None` if there's no parseable volume line.
/// Receivers send volume as `text/parameters` (`volume: -20.00\r\n`), the same
/// shape we send; the reverse channel echoes/reports it.
fn extract_volume(buf: &[u8]) -> Option<f32> {
    let text = std::str::from_utf8(buf).ok()?;
    let line = text
        .lines()
        .rev()
        .find(|l| l.trim_start().to_ascii_lowercase().starts_with("volume:"))?;
    let db: f32 = line.split(':').nth(1)?.trim().parse().ok()?;
    Some(db_to_linear(db))
}

/// Read the receiver's reverse event channel, decrypt each HomeKit-framed block
/// (2-byte LE length + ciphertext + 16-byte tag) with the read key on a fresh
/// nonce stream — exactly the inbound framing the RTSP channel uses — and forward
/// any device-reported volume (0.0–1.0) on `tx`.
///
/// Fail-safe by construction: it owns the event socket and a decrypt-only cipher,
/// touches nothing else, and returns (ending the task) on EOF, a decrypt failure,
...
async fn event_reader_loop(mut stream: TcpStream, read_key: [u8; 32], tx: mpsc::UnboundedSender<f32>) {
    ...
    loop {
        ...
        if stream.read_exact(&mut len_buf).await.is_err() {
            break; // EOF / receiver closed the channel
        }
        let block_len = u16::from_le_bytes(len_buf) as usize;
        if block_len == 0 || block_len > 8192 {
            tracing::debug!("AP2 event channel: implausible block length {block_len}; stopping reader");
            break;
        }
        let mut cipher_block = vec![0u8; block_len + 16];
        if stream.read_exact(&mut cipher_block).await.is_err() {
            break;
        }
        match cipher.decrypt_block(&cipher_block, block_len as u16) {
            Ok(plain) => {
            ...
            Err(e) => {
                tracing::debug!("AP2 event channel decrypt failed ({e}); stopping reader");
                break;
            }
        }
    }
    tracing::debug!("AP2 event-channel reader exiting");
}
...
mod tests {
    ...
    mod event_channel_volume {
        use super::*;

        #[test]
        fn db_to_linear_maps_endpoints_and_floor() {
            assert!((db_to_linear(0.0) - 1.0).abs() < 1e-6); // 0 dB = full scale
            assert_eq!(db_to_linear(-144.0), 0.0); // AirPlay mute sentinel
            assert_eq!(db_to_linear(-30.0), 0.0); // floor
            let mid = db_to_linear(-6.0); // ~0.501
            assert!(mid > 0.45 && mid < 0.55, "got {mid}");
        }

        #[test]
        fn extract_volume_parses_last_volume_line() {
            let msg = b"SET_PARAMETER rtsp://x RTSP/1.0\r\nContent-Type: text/parameters\r\n\r\nvolume: -6.00\r\n";
            let v = extract_volume(msg).expect("volume parsed");
            assert!((v - db_to_linear(-6.0)).abs() < 1e-6);
        }

        #[test]
        fn extract_volume_none_without_a_volume_line() {
            assert!(extract_volume(b"SET_PARAMETER rtsp://x RTSP/1.0\r\nprogress: 1/2/3\r\n").is_none());
        }
    }
