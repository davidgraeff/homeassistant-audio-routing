...
async fn async_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Multi-room: the primary's single libairptp grandmaster must drive EVERY
    // receiver. Primary setup already added device 1 (its own addr); add the rest.
    for (i, (ip, _)) in targets.iter().enumerate().skip(1) {
        connections[0].ptp_add_peer(&ip.to_string());
        println!("  Added device {} ({}) as PTP peer of the primary master", i + 1, ip);
    }
