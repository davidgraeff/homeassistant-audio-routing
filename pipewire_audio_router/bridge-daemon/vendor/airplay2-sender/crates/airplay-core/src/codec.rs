...
impl AudioFormat {
    ...
    /// AirPlay SETUP `audioFormat` bit for this exact (codec, rate, bit-depth).
    ///
    /// The AirPlay audioFormat bitfield encodes codec+rate+depth+channels, and the
    /// receiver uses it to interpret the stream — so it MUST agree with the ALAC
    /// magic-cookie's rate (a 48 kHz stream declared as the 44.1 kHz bit plays at
    /// the wrong pitch or is rejected). `AudioCodec::audio_format_value` is
    /// rate-blind (always the 44.1 kHz value); prefer this. ALAC/stereo entries:
    ///   bit18 0x40000 ALAC/44100/16/2   bit19 0x80000 ALAC/44100/24/2
    ///   bit20 0x100000 ALAC/48000/16/2  bit21 0x200000 ALAC/48000/24/2
    pub fn airplay_format_value(&self) -> u32 {
        match (self.codec, self.sample_rate, self.bit_depth) {
            (AudioCodec::Alac, SampleRate::Hz44100, 16) => 0x40000,
            (AudioCodec::Alac, SampleRate::Hz44100, 24) => 0x80000,
            (AudioCodec::Alac, SampleRate::Hz48000, 16) => 0x100000,
            (AudioCodec::Alac, SampleRate::Hz48000, 24) => 0x200000,
            // AAC / AAC-ELD / Opus: rate handling out of scope here — the
            // codec-level value is what the buffered paths already used.
            _ => self.codec.audio_format_value(),
        }
    }
