...
impl PtpHeader {
    ...
    pub fn serialize(&self) -> [u8; 34] {
        ...
        // AirPlay 2 timing is gPTP (IEEE 802.1AS): the high nibble of byte 0 is
        // majorSdoId and MUST be 0x1. Third-party receivers silently drop any PTP
        // frame that isn't gPTP-tagged, so with 0x0 here they never lock.
        buf[0] = 0x10 | (self.message_type as u8);
        buf[1] = self.version;
        buf[2..4].copy_from_slice(&self.message_length.to_be_bytes());
        buf[4] = self.domain_number;
        buf[5] = 0; // Reserved
        // Match OwnTone: always advertise PTP_UNICAST (0x0400) + PTP_TIMESCALE
        // (0x0008), preserving any TWO_STEP (0x0200) the caller set.
        let flags = self.flags | 0x0400 | 0x0008;
...
pub async fn send_ptp_announce(
    ...
    *announce_seq = announce_seq.wrapping_add(1);
    ...
    header.source_port_identity = source_port_identity;
    // Match OwnTone's Announce byte-for-byte (76 bytes incl. a Path Trace TLV).
    header.message_length = 76; // 34 header + 10 ts + 20 body + 12 Path Trace TLV
    header.log_message_interval = 0;

    let mut packet = [0u8; 76];
    packet[..34].copy_from_slice(&header.serialize());

    // Bytes 34-43: originTimestamp — zero
    // Bytes 44-45: currentUtcOffset — 0 (OwnTone leaves this 0)
    // Byte 46: reserved
    // Byte 47: grandmasterPriority1 — lower wins BMCA
    packet[47] = priority1;
    // Bytes 48-51: grandmasterClockQuality — matches OwnTone exactly:
    //   clockClass, accuracy=0x21, offsetScaledLogVariance=0x436a
    packet[48] = clock_class;
    packet[49] = 0x21;
    packet[50..52].copy_from_slice(&0x436au16.to_be_bytes());
    // Byte 52: grandmasterPriority2 — 128
    packet[52] = 128;
    // Bytes 53-60: grandmasterIdentity — our clock identity
    packet[53..61].copy_from_slice(clock_identity);
    // Bytes 61-62: stepsRemoved — 0 (we are the grandmaster)
    // Byte 63: timeSource — 0x20 (matches OwnTone)
    packet[63] = 0x20;
    // Bytes 64-75: Path Trace TLV (type 0x0008, len 0x0008, path = our clock id)
    packet[64..66].copy_from_slice(&0x0008u16.to_be_bytes());
    packet[66..68].copy_from_slice(&0x0008u16.to_be_bytes());
...
pub async fn send_ptp_sync(
    ...
    *sequence_id = sequence_id.wrapping_add(1);
    ...
    // Send Sync message (event port) - two-step: timestamp in Follow_Up
    let mut sync_header = PtpHeader::new(PtpMessageType::Sync, seq);
    sync_header.flags = 0x0200; // Two-step flag (serialize() adds UNICAST+TIMESCALE)
    sync_header.log_message_interval = -3; // 2^-3 = 125ms (8 Hz), matches OwnTone
    sync_header.source_port_identity = source_port_identity;
    ...
    // Send Follow_Up message (general port) with precise timestamp + gPTP TLVs.
    // OwnTone attaches BOTH the standard 802.1AS Follow_Up Info TLV AND an Apple
    // proprietary TLV (OUI 00:0D:93, subtype 4, payload = GM clock id + 0x0000).
    let mut followup_header = PtpHeader::new(PtpMessageType::FollowUp, seq);
    followup_header.log_message_interval = -3;
    followup_header.source_port_identity = source_port_identity;

    // Standard 802.1AS Follow_Up Information TLV (OUI 00:80:C2, subtype 1).
    let followup_tlv = FollowUpInformationTlv::new();
    let tlv_bytes = followup_tlv.serialize();

    // Apple proprietary Follow_Up TLV (OUI 00:0D:93, subtype 4).
    let mut apple_payload = Vec::with_capacity(10);
    apple_payload.extend_from_slice(clock_identity);
    apple_payload.extend_from_slice(&[0x00, 0x00]);
    let apple_tlv = AppleTlv::new([0x00, 0x00, 0x04], apple_payload);
    let apple_bytes = apple_tlv.serialize();

    // Update message length to include both TLVs (→ 96 bytes total, like OwnTone).
    followup_header.message_length = 44 + tlv_bytes.len() as u16 + apple_bytes.len() as u16;

    let mut followup_packet = Vec::new();
    followup_packet.extend_from_slice(&followup_header.serialize());
    followup_packet.extend_from_slice(&sync_time.serialize());
    followup_packet.extend_from_slice(&tlv_bytes);
    followup_packet.extend_from_slice(&apple_bytes);
...
pub async fn run_bmca_yield_flow(
    ...
    for i in 0..3 {
        ...
        if i < 2 {
            send_ptp_announce(&general_socket, general_dest, &clock_identity, &mut announce_seq, 6, priority1).await?;
        }
    ...
    loop {
        ...
        if announce_interval_counter >= announce_every {
            ...
            for &general_dest in &general_dests {
                if let Err(e) = send_ptp_announce(&general_socket, general_dest, &clock_identity, &mut announce_seq, 6, priority1).await {
        ...
        loop {
            tokio::select! {
                    ...
                    if let Ok((len, src)) = result {
                        if let Ok(header) = PtpHeader::parse(&event_buf[..len]) {
                            if header.message_type == PtpMessageType::DelayReq {
                                let recv_time = PtpTimestamp::now();
                                tracing::info!("PTP master: Delay_Req from {} (seq={}) — DEVICE IS SLAVING TO US", src, header.sequence_id);
                ...
                result = general_socket.recv_from(&mut general_buf) => {
                    if let Ok((len, src)) = result {
                        if let Ok(header) = PtpHeader::parse(&general_buf[..len]) {
                            tracing::info!("PTP master: Received {:?} on general port from {}", header.message_type, src);
                        }
                    }
                }
            }
