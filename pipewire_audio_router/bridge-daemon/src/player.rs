...
/// Plays a 16-bit PCM WAV clip to the sink node `target_node_id`, blocking
/// until the clip has fully drained (or a safety timeout fires). Intended to
/// be run on a blocking thread.
pub fn play_wav_to_target(target_node_id: u32, wav_bytes: &[u8]) -> Result<(), String> {
    ...
    let n_frames = wav.pcm.len() / stride;
    let clip = Duration::from_secs_f64(n_frames as f64 / wav.rate.max(1) as f64);
    ...
    let stream = pw::stream::StreamBox::new(
        ...
        pw::properties::properties! {
            ...
            *pw::keys::NODE_NAME => "bridge-announce",
        },
    )
    ...
    let _listener = stream
        ...
        .process({
            ...
            move |stream, _| {
                ...
                let filled = if let Some(slice) = data.data() {
                    ...
                    let cur = cursor.get();
                    let remaining_frames = pcm.len().saturating_sub(cur) / stride;
                    let n = cap_frames.min(remaining_frames);
                    let nbytes = n * stride;
                    slice[..nbytes].copy_from_slice(&pcm[cur..cur + nbytes]);
                    cursor.set(cur + nbytes);
                    nbytes
                } else {
                    0
                };
                ...
                *chunk.size_mut() = filled as _;
                // All PCM queued: ask the stream to drain once; the `drained`
                // callback then quits the loop after the tail has played out.
                if cursor.get() >= pcm.len() && !draining.get() {
                    draining.set(true);
        ...
        .drained({
            let mainloop = mainloop.clone();
            move |_stream, _| mainloop.quit()
        })
        .register()
        .map_err(|e| format!("register stream listener: {e}"))?;

    // Format: S16LE at the clip's rate/channels (FL/FR positions for stereo).
    ...
    // Safety net: a sink that never drains (e.g. a stuck RAOP device) must not
    // hang this thread forever. The loop is `!Send`, so an external timeout
    // can't call `quit()` — arm an in-loop timer instead.
    let timed_out = Rc::new(Cell::new(false));
    let timer = {
        let ml_for_timer = mainloop.clone();
        let timed_out = timed_out.clone();
        // `.loop_()` borrows the outer `mainloop` (lives to end of fn); the
        // closure captures its own clone so it doesn't extend that borrow.
        mainloop.loop_().add_timer(move |_| {
            timed_out.set(true);
            ml_for_timer.quit();
        })
    };
    let _ = timer.update_timer(Some(clip + Duration::from_secs(5)), None);
