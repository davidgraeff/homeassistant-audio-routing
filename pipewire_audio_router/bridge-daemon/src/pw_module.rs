...
//! PipeWire thread (pw_thread.rs). Everything here must be called on that
//! thread. `LoadedModule` holds a raw pointer and is therefore `!Send`, so the
//! compiler prevents it from escaping that thread — including its `Drop`, which
//! calls back into libpipewire.
...
/// destroys every node it created — which is exactly how removing a RAOP
/// output makes its sink node disappear from the graph, live, with no restart.
pub struct LoadedModule {
    ptr: *mut pw_impl_module,
}

impl LoadedModule {
    /// Load `name` (e.g. `libpipewire-module-raop-sink`) with `args` (an
    /// SPA-JSON object string, see `raop::raop_module_args`) into `context`.
    ///
    /// # Safety
    /// `context` must be a live `pw_context` owned by the current (PipeWire)
    /// thread, and this must be called on that thread. `NUL` bytes in `name`/
    /// `args` are rejected rather than passed to C.
    pub unsafe fn load(context: *mut pw_context, name: &str, args: &str) -> Result<Self, String> {
        ...
        // The module takes ownership of nothing we pass; passing NULL props
        // lets it build its own from `args`, matching config-file loading.
...
impl Drop for LoadedModule {
    fn drop(&mut self) {
        // Only ever constructed and dropped on the PipeWire thread (LoadedModule
        // is !Send via the raw pointer), so this is safe to call here.
