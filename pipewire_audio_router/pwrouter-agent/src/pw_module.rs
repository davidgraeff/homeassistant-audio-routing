...
//! runtime.
//!
//! Copied verbatim (minus doc examples) from
//! `bridge-daemon/src/pw_module.rs` — the agent needs exactly the same trick on
//! the receiver side, and the two crates do not share a workspace yet. P1 folds
//! this into the shared `pw-control` crate (docs/receiver-agent-plan.md §12);
//! until then, changes must be mirrored.
//!
...
/// destroys every node it created — which is how the agent's receive stream
/// disappears from the host's graph the moment the agent exits, with no
/// PipeWire restart and no stale config (plan §7).
