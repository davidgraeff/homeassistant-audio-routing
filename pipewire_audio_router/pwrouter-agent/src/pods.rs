//! SPA pod encode/decode for the two volume levers the agent uses, plus the
//! cubic↔linear scale.
//!
//! Mirrors the pod handling in `bridge-daemon/src/volume.rs`; kept as a marked
//! copy until the shared `pw-control` crate exists (docs/receiver-agent-plan.md
//! §13 — the `Dockerfile` build context blocks the extraction).
//!
//! ## Two levers, deliberately
//!
//! * **Device `Route`** — the master volume/mute of a real device sink. This is
//!   what `wpctl` and the desktop applet read and write, so it is what the HA
//!   slider must drive (plan §6.1: node `Props` writes on a device sink are
//!   invisible *and* get re-synced away by WirePlumber).
//! * **Node `Props`** — per-stream gain. Correct for ducking *other*
//!   applications' streams, where staying out of the user's device slider is a
//!   feature, not a bug.
//!
...
fn serialize(value: &Value) -> anyhow::Result<Vec<u8>> {
    Ok(PodSerializer::serialize(std::io::Cursor::new(Vec::new()), value)
...
pub fn route_pod(index: i32, device: i32, linear: &[f32], mute: Option<bool>) -> anyhow::Result<Vec<u8>> {
    let mut inner = vec![Property::new(
        spa_sys::SPA_PROP_channelVolumes,
        Value::ValueArray(ValueArray::Float(linear.to_vec())),
    )];
    if let Some(m) = mute {
